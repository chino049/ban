    ...
    def run(self):
        for row in self.stock_rows:
            ...
            try:
                ...
                self.signals.price_result.emit(symbol, price, prev, change)
    ...
    def run(self):
        for row in self.stock_rows:
            ...
            try:
                ...
                w_diff = w_pct = 0
                if not d.empty:
                    start, end = float(d["Close"].iloc[0]), float(d["Close"].iloc[-1])
                    w_diff = end - start
                ...
                self.signals.wms_result.emit(symbol, w_diff, w_pct,
                                             m_diff, m_pct, s_diff, s_pct)
            except Exception as e:
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # ----- row list (symbol, price, prev, week, month, sem, ?, ?, button) -----
        self.rows = [
    ...
    def handle_price(self, sym, price, prev, change):
        for r in self.rows:
            ...
            # ---- button background (hsX) ----
            self.set_button_bg(r[8], change)

            # ---- price text (le?p) – always white ----
            r[1].setStyleSheet("color:white; font-weight:bold;")
            r[1].setText(f"{change:+.2f}%   {price:.2f}$")

            # ---- previous close ----
            r[2].setText(f"{prev:.2f}$")
    ...
    def handle_wms(self, sym, w_d, w_p, m_d, m_p, s_d, s_p):
        for r in self.rows:
            if r[0].text().strip() != sym:
                continue
            # week
            r[3].setStyleSheet(f"color:{self.color_for(w_p)};")
            r[3].setText(f"{w_d:+.2f}$   {w_p:+.2f}%")
            # month
            r[4].setStyleSheet(f"color:{self.color_for(m_p)};")
            r[4].setText(f"{m_d:+.2f}$   {m_p:+.2f}%")
            # semester
            r[5].setStyleSheet(f"color:{self.color_for(s_p)};")
    ...
    def set_button_bg(self, btn, pct):
        """Only the price-button (hsX) changes background."""
        styles = {
            "lime":    "background:#00ff00; color:white; border-radius:8px; padding:4px; font-weight:bold;",
            "green":   "background:#00cc00; color:white; border-radius:8px; padding:4px; font-weight:bold;",
            "gray":    "background:#333333; color:white; border-radius:8px; padding:4px;",
            "red":     "background:#ff0000; color:white; border-radius:8px; padding:4px; font-weight:bold;",
            "darkred": "background:#cc0000; color:white; border-radius:8px; padding:4px; font-weight:bold;",
            "orange":  "background:#ffaa00; color:white; border-radius:8px; padding:4px; font-weight:bold;",
        }
...
if __name__ == "__main__":
    ...
    # ---- global stylesheet (progress bar & control buttons keep dark look) ----
    app.setStyleSheet("""
