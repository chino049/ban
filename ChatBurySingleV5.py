...
from PyQt5.QtCore import Qt, QThreadPool, QRunnable, pyqtSignal, QObject, QMetaObject
from PyQt5.QtGui import QPalette, QColor
import yfinance as yf
from qtd3 import Ui_MainWindow  # Ensure this file exists (from Qt Designer)
...
# -------------------------------
# W/M/S Update Worker (FIXED)
# -------------------------------
class WMSUpdateWorker(QRunnable):
    def __init__(self, stock_rows):
        super().__init__()
        self.stock_rows = stock_rows  # Store the rows
        self.signals = WorkerSignals()

    def run(self):
        for row in self.stock_rows:  # Use self.stock_rows
            symbol = row[0].text().strip()
            if not symbol:
                continue
            try:
...
class MainWindow(QtWidgets.QMainWindow, Ui_MainWindow):
    def __init__(self):
        ...
        # Tooltips
        self.startButton.setToolTip("Start live price updates (every 60s)")
        ...
        self.st = [
            ...
            [self.le2s, self.le2p, self.le2c, self.le2w, self.le2m, self.le2t, self.le2f, fself.le2l, self.hs2],
    ...
    def handle_price_update(self, symbol, price, prev, change):
        for row in self.st:
            if row[0].text().strip() == symbol:
                ...
                # Update price
                color = self.get_change_color(change)
                row[1].setStyleSheet(f"color: {color}; font-weight: bold;")
    ...
    def populate_week_month_semester(self):
        ...
        worker = WMSUpdateWorker(self.st)  # Pass self.st
    ...
    def update_button_color(self, button, change):
        styles = {
            "lime":     "background-color: lime; color: black; border-radius: 8px; font-weight: bold;",
            "green":    "background-color: #00ff00; color: black; border-radius: 8px; font-weight: bold;",
            "white":    "background-color: #333333; color: white; border-radius: 8px;",
            "red":      "background-color: red; color: white; border-radius: 8px; font-weight: bold;",
            "darkred":  "background-color: #ff4444; color: white; border-radius: 8px; font-weight: bold;",
            "orange":   "background-color: orange; color: black; border-radius: 8px; font-weight: bold;"
        }
        key = "white"
        if change > 3:
            key = "lime"
            ...
            key = "orange"
        button.setStyleSheet(styles[key])
